...
This plugin ships via the **ACX team marketplace**. An admin imports the repo once; PMs install from Customize.

1. **Admin**: Dashboard → Settings → Plugins → Import Marketplace → `https://github.com/timobuilds/acx-pm`
2. **PM**: Customize → install **acx-pm** from the team marketplace
...
Copy `skills/` into a project's `.cursor/skills/` or globally into `~/.cursor/skills/`. Copy `CONVENTIONS.md` alongside them and keep the relative `../../CONVENTIONS.md` reference resolvable.

### Claude Code / Cowork

Install the `acx-pm/` directory via Claude's plugin flow — it picks up `.claude-plugin/plugin.json` automatically. MCP requirements and skill behavior are identical to Cursor.

## Maintaining

See the [marketplace README](../README.md) and root `CHANGELOG.md` for release steps.
